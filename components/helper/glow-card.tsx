...
const GlowCard = ({ identifier, children }: any) => {
  useEffect(() => {
    ...
    const UPDATE = (event: PointerEvent) => {
      // CARDS NodeList üzerinden forEach ile geçiyoruz
      CARDS.forEach((card) => {
        const CARD = card as HTMLElement; // Tür dönüşümü yapıyoruz
    ...
    // `UPDATE()` doğrudan çağırılmayacak, çünkü event geçilmediğinde hata alırız.
    // UPDATE();
  ...
  return (
    <div className={`glow-container-${identifier} glow-container`}>
      <article
        className={`glow-card glow-card-${identifier} h-fit cursor-pointer border border-[#2a2e5a] transition-all duration-300 relative bg-[#101123] text-gray-300 rounded-xl hover:border-transparent w-full`}>
